...
function bdw_get_images_with_info($iPostID,$img_size='thumb'){
		...
		if($arrImages){
			...
			if (has_post_thumbnail( $iPostID )){
				$img_arr = wp_get_attachment_image_src( get_post_thumbnail_id( $iPostID ), 'small_slider_thumbs' );
				$imgarr['id'] = $id;
				$imgarr['file'] = $img_arr[0];
				$return_arr[] = $imgarr;
			}
...
function post_type_meta(){
	wp_enqueue_script('jQueryversion','http://ajax.googleapis.com/ajax/libs/jquery/1.5/jquery.min.js'); ?>
