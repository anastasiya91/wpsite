...
class dc_jqmegamenu_widget extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		function load_megamenu(){
			wp_enqueue_script('jquerymegamenu', get_stylesheet_directory_uri()."/js/jquery.megamenu.1.2.js");
			wp_enqueue_script('jquerymegamenuhoverint', get_stylesheet_directory_uri()."/js/jquery.hoverIntent.minified.js");
		}
		...
		foreach ($all_widgets as $key => $wpdcjqmegamenu){
			...
			//var $mgc = jQuery.noConflict();
				jQuery(document).ready(function() {
					jQuery('#<?php echo $widget_id.'-item'; ?> .mega').dcMegaMenu({
