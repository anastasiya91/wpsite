...
if(function_exists('register_sidebars')){
	register_sidebars(1,array('id' => 'mega_menu', 'name' => 'JQuery Mega Menu', 'description' => 'Display jquery mega menu.', 'before_widget' => '<div class="widget">', 'after_widget' => '</div>','before_title' => '<h3>', 'after_title' => '</h3>'));
	register_sidebars(1,array('id'=>'header_search','name'=>'Header Search','description'=>'Display Search wiget in Navigation menu','before_widget'=>'<div class="widget" style="margin-top:-10px;">','after_widget'=>'</div>','before_title'=>'<h3>','after_title'=>'</h3>'));
...
if(!class_exists('subscribewidget')){
	class subscribewidget extends WP_Widget {
		...
		//Constructor
			$widget_ops = array('classname' => 'widget Newsletter Subscribe', 'description' => __('Display Newsletter Subscribe Widget. Best use in "footer1" widget area.') );		
			$this->WP_Widget('widget_subscribewidget', __('T &rarr; Newsletter Subscribe'), $widget_ops);
		}
...
if(!class_exists('text_widget')){
	class text_widget extends WP_Widget {
		...
		//Constructor
			$widget_ops = array('classname' => 'text_widget', 'description' => __('Displays simple text widget with read more link. Best use in sidebar or footer widget areas.') );		
			$this->WP_Widget('text_widget', __('T &rarr; Text Widget'), $widget_ops);
		}
		function widget($args, $instance) {
		// prints the widget
			extract($args, EXTR_SKIP);
	...
	?>
			...
			<p>
			  <label for="<?php echo $this->get_field_id('read_more_text1'); ?>"><?php _e('Read more text1');?>
			...
			<p>
			  <label for="<?php echo $this->get_field_id('read_more_link1'); ?>"><?php _e('Read more link1');?>
...
if(!class_exists('woo_shopping_cart_info')){
	class woo_shopping_cart_info extends WP_Widget {
		...
		//Constructor
			$widget_ops = array('classname' => 'widget WooCommerce shopping cart info', 'description' => __('Display Cart Informations with automatic cart update. Best to use it in "Header right area" widget are.'));
			$this->WP_Widget('woo_shopping_cart_info', __('T &rarr; WooCommerce Shopping Cart'), $widget_ops);
		}
		...
		function widget($args, $instance) {
			...
			<div  id="woo_shoppingcart_box" class="shoppingcart_box" style="cursor:pointer;width:200px;height:60px;">
				<div class="shoppingcart_box_bg">
							...
							if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) {
								...
								foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
									...
									if ( $_product->exists() && $cart_item['quantity'] > 0 ) {
										$woo_cart_total += $cart_item['quantity'];
									}
								}
								if($woo_cart_total == 1){
									$woo_items = "item";
								...
								_e($woo_cart_total." <span class='woo_cart_desc'>".$woo_items." in your cart</span>","templatic");
							}else{
								_e("0 <span class='woo_cart_desc'>item in your cart</span>","templatic");
							}
						?>
					...
					<?php 	
						echo '<p class="cart_checkout"><b>' . $woocommerce->cart->get_cart_subtotal().'</b>';
...
if(!class_exists('banner_widget')){
	class banner_widget extends WP_Widget {
		...
		//Constructor
			$widget_ops = array('classname' => 'widget_advertisement', 'description' => __('Set Advertisement either banner or Google Ad Sence Code. Best Use in "Header Advertisement" Widget area.') );
			$this->WP_Widget('widget_advertisement', __('T &rarr; Set Advertisement'), $widget_ops);
		}
	...
	?>
			...
			<p>
			  <label for="<?php echo $this->get_field_id('advertisement'); ?>"><?php _e('Advertisement:<small>(eg:google adsense code)</small>');?> </label>
...
//remove_action('posts_where','templatic_searching_filter_where');
	...
	<?php	
			...
			function widget($args,$instance){
				...
				$get_post_id_by_title = $wpdb->get_var( $wpdb->prepare($sql_query));
						...
						?>
									<a class="post_img" href="<?php the_permalink(); ?>">
										<?php
											...
											<img  src="<?php echo $post_images[0]['file'];?>" alt="<?php $alt; ?>" title="<?php echo $title; ?>" width="220" height="220" />
										<?php }else {?>
			  ...
			  <div class="flex-flexslider">
				...
				?>
					...
					<ul class="flex-slides">
						   ...
						   <?php /*do_action('woocommerce_before_shop_loop_item_title'); */  $post_images =  bdw_get_images_with_info($post->ID,'small_slider_thumbs'); ?>
			...
			?>
				<script type="text/javascript">
					jQuery(document).ready(function($) {
						$('input[id=<?php echo $this->get_field_id('is_Carousel'); ?>]').click(function(){
							$('#<?php echo $this->get_field_id('home_slide_carousel'); ?>').slideToggle('slow');
						});
					});
					jQuery(document).ready(function($) {
...
?>style="width:10px;" onclick=""/> <?php _e("<b>Settings for Carousel slider option?</b>",
"templatic");?>	
					</label>
				</p>
				<div id="<?php echo $this->get_field_id('home_slide_carousel'); ?>" style="<?php if($is_Carousel =='1'){ ?>display:block;<?php }else{?>display:none;<?php }?>">
					<p><?php _e("<small><b>This options are only works if Animation type is Slide</b></small>");?></p>
					<p>
				...
				<p><br/>
				  <label for="<?php echo $this->get_field_id('custom_banner'); ?>">
					<input id="<?php echo $this->get_field_id('custom_banner'); ?>" name="<?php echo $this->get_field_name('custom_banner'); ?>" type="checkbox" value="1" <?php if($custom_banner =='1'){ ?>checked=checked<?php } ?>style="width:10px;" onclick="" /> <?php _e('<b>Use custom images?</b>','templatic');?>	<br/> 
				  </label><br/>
				</p>
				...
				<div id="<?php echo $this->get_field_id('home_slide_custom'); ?>" style="<?php if($custom_banner =='1'){ ?>display:block;<?php }else{?>display:none;<?php }?>">
					<p><label for="<?php echo $this->get_field_id('s1'); ?>"><?php _e('Banner Slider Image 1 full URL <small>(ex.http://templatic.com/images/banner1.png, Image size 980x425 )</small>  :');?> 
					...
					<p><label for="<?php echo $this->get_field_id('s1link'); ?>"><?php _e('Banner Slider Image 1 Link <small>(ex.http://templatic.com)</small>  :');?> 
...
/* =============================== Category listing widget START ====================================== */
	if(!class_exists('browse_by_categories')){
		class browse_by_categories extends WP_Widget {
				...
				//Constructor
					$widget_ops = array('classname' => 'widget browse_by_cats', 'description' => __('Display all product categories. Use it in sidebar areas.') );
					$this->WP_Widget('browse_by_cats', __('T &rarr; Browse By Categories'), $widget_ops);
				}
...
if(file_exists(include(TEMPLATE_FUNCTION_FOLDER_PATH.'mega_menu_widget.php'))){
	include(TEMPLATE_FUNCTION_FOLDER_PATH.'mega_menu_widget.php');
}
